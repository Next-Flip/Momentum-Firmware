...
bool csc_parse(const NfcDevice* device, FuriString* parsed_data) {
    ...
    do {
        ...
        const uint8_t* current_balance_block_start_ptr = &data->block[current_balance_block_num].data[0];
        const uint8_t* current_balance_copy_block_start_ptr = &data->block[current_balance_copy_block_num].data[0];
        uint32_t current_balance_and_times = bit_lib_bytes_to_num_le(current_balance_block_start_ptr, 4);
        uint32_t current_balance_and_times_copy = bit_lib_bytes_to_num_le(current_balance_copy_block_start_ptr, 4);
        ...
        if(current_balance_and_times != current_balance_and_times_copy) {
            ...
            break;} // failed verification if balance != backup
        if(current_balance_and_times == 0 || current_balance_and_times_copy == 0) {
            FURI_LOG_D(TAG, "Value bytes empty");
            break;} // even if balance = 0, e.g. new card, refilled times can't be zero
        // Parse data 
        ...
        uint32_t card_lives = bit_lib_bytes_to_num_le(card_lives_block_start_ptr, 2); // how many times it can still be used
        ...
        if(refill_sign == 0 && refill_times == 1) { // new cards don't comply to checksum but refill time should be once
            furi_string_printf(
                parsed_data,
