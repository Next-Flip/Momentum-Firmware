...
static int offset_to_block(int offset) { return offset / 16; }

static int offset_in_block(int offset) { return offset % 16; }
...
static bool authenticate_and_read(Nfc *nfc, uint8_t sector, const uint8_t *key,
    ...
    MfClassicError error = mf_classic_poller_sync_auth(
        nfc, mf_classic_get_first_block_num_of_sector(sector), &mf_key,
        key_type, NULL);
    if (error != MfClassicErrorNone) {
        FURI_LOG_D(TAG, "Authentication failed for sector %d key type %d",
                   sector, key_type);
        return false;
    }
...
static bool parse_trip_data(const MfClassicBlock *block_data, TripData *trip,
                            int trip_offset) {
    trip->timestamp =
        bit_lib_bytes_to_num_le(block_data->data + trip_offset + 3, 4);
    trip->tap_on = (block_data->data[trip_offset + 7] & 0x10) == 0x10;
    memcpy(trip->route, block_data->data + trip_offset + 8, 4);
    trip->route[4] = '\0';
    trip->cost =
        bit_lib_bytes_to_num_le(block_data->data + trip_offset + 13, 2);
    trip->transaction_number =
        bit_lib_bytes_to_num_le(block_data->data + trip_offset, 2);
    trip->journey_number =
        bit_lib_bytes_to_num_le(block_data->data + trip_offset + 2, 2);
    return true;
}
...
static bool smartrider_parse(const NfcDevice *device, FuriString *parsed_data) {
    ...
    do {
        ...
        if (sec_tr == NULL) {
            FURI_LOG_E(TAG, "Failed to get sector trailer for sector %d",
                       verify_sector);
            break;
        }
        const uint64_t key = bit_lib_bytes_to_num_be(
            sec_tr->key_a.data, COUNT_OF(sec_tr->key_a.data));
        if (key != bit_lib_bytes_to_num_be(STANDARD_KEYS[0],
                                           sizeof(STANDARD_KEYS[0]))) {
        ...
        const uint8_t required_blocks[] = {
            offset_to_block(0xe0),  offset_to_block(0x40),
            offset_to_block(0x50),  1,
            offset_to_block(0x340), offset_to_block(0x320),
            offset_to_block(0x06)};
        for (size_t i = 0; i < COUNT_OF(required_blocks); i++) {
            if (!mf_classic_is_block_read(data, required_blocks[i])) {
                FURI_LOG_E(TAG, "Required block %d is not read",
                           required_blocks[i]);
                break;
            }
        }
        ...
        sr_data.balance =
            bit_lib_bytes_to_num_le(block_data + offset_in_block(0xe0) + 7, 2);
        ...
        sr_data.auto_load_threshold =
            bit_lib_bytes_to_num_le(block_data + 20, 2);
        ...
        snprintf(sr_data.card_serial_number, sizeof(sr_data.card_serial_number),
                 "%02X%02X%02X%02X%02X", block_data[6], block_data[7],
                 block_data[8], block_data[9], block_data[10]);
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (!parse_trip_data(&data->block[trip_block],
                                 &sr_data.last_trips[i],
                                 offset_in_block(trip_offsets[i]))) {
                FURI_LOG_E(TAG, "Failed to parse trip data for offset 0x%03X",
                           trip_offsets[i]);
                break;
            }
        }
        ...
        furi_string_printf(
            ...
            strncmp(sr_data.card_serial_number, "00", 2) == 0
                ? sr_data.card_serial_number + 2
